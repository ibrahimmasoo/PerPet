  ...
  try {
    ...
    if (code === 'permission-denied') {
      setAdminGateMessage(
        ...
        'Your admin account is signed in, but Firestore is blocking dashboard access. Update your Firestore rules to allow the admin email to read and write siteContent, pets, analytics, users, and contactMessages.'
      );
    } else {
      setAdminGateMessage(
        'Could not load admin dashboard',
        'Open the browser console or check Firebase rules. Error: ' + (code || error.message || 'unknown')
      );
    }
    hideAdminApp();
  }
...
if (adminAuth) {
  adminAuth.onAuthStateChanged(async user => {
    ...
    if (!isAdmin(user)) {
      ...
      hideAdminApp();
      if (adminAuthBtn) adminAuthBtn.textContent = 'Sign Out';
      return;
    }
