...
function updateAuthUI(user) {
  ...
  if (currentUser) {
    const name = currentUser.displayName ? currentUser.displayName.split(' ')[0] : 'Account';
...
async function signInWithEmail(email, password) {
  ...
  try {
    ...
    showToast(getFriendlyAuthError(error));
  }
}

async function createEmailAccount(firstName, lastName, email, password, confirmPassword) {
  if (!auth) {
    showToast('Firebase is not loaded.');
    return;
  }
  ...
  try {
    ...
    setTimeout(() => {
      window.location.href = 'sign-in.html?registered=1';
    }, 700);
  } catch (error) {
...
function signInWithGoogle() {
  ...
  auth.signInWithPopup(provider)
    .then((result) => {
      ...
      if (window.location.pathname.endsWith('sign-in.html') || window.location.pathname.endsWith('create-account.html')) {
        setTimeout(() => {
          window.location.href = 'index.html';
        }, 500);
      }
...
if (createAccountForm) {
  createAccountForm.addEventListener('submit', event => {
    ...
    createEmailAccount(firstName, lastName, email, password, confirmPassword);
  });
}

if (window.location.search.includes('registered=1')) {
  setTimeout(() => {
    showToast('Account created successfully. Sign in to continue.');
  }, 200);
}
