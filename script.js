...
function capturePetLocationForListing() {
  ...
  navigator.geolocation.getCurrentPosition(
    async position => {
      ...
      setPetFormLocation(latitude, longitude);

      if (petLocationStatus) {
        petLocationStatus.textContent = 'Getting address details...';
      }
      ...
      try {
        ...
        const cityInput = document.getElementById('petCityInput');
        ...
        if (petLocationStatus) {
          petLocationStatus.textContent = 'Location and address added successfully.';
        }
      } catch (geoError) {
        console.error('Reverse geocoding failed:', geoError);

        if (petLocationStatus) {
          petLocationStatus.textContent = 'Location added successfully, but address could not be detected automatically.';
        }
      }
    },
    error => {
  ...
  prepared.forEach(pet => {
    ...
    card.innerHTML = `
      ...
      <div class="pet-body">
        ...
        <button class="btn btn-dark adopt-btn" data-pet-id="${pet.id}" type="button">Adopt ${escapeHtml(pet.name)}</button>
      </div>
    `;
...
if (petForm) {
  petForm.addEventListener('submit', async event => {
    ...
    // ── Save to Firestore and wait until save completes ──
    if (db && typeof firebase !== 'undefined' && firebase.firestore) {
      try {
        ...
        console.error('Could not save pet to Firestore:', error);
        showToast('Could not save pet to database.');
        submitBtn.disabled = false;
        submitBtn.textContent = 'Post Pet for Adoption';
        return;
      }
