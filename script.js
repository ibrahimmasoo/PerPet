  ...
  prepared.forEach(pet => {
    ...
    card.innerHTML = `
      ...
      <div class="pet-body">
        ...
        <button class="btn btn-dark adopt-btn" data-id="${pet.id}" type="button">Adopt ${escapeHtml(pet.name)}</button>
        ${currentUser && pet.ownerId===currentUser.uid ? `<button class="btn btn-light delete-pet-btn" data-id="${pet.id}" type="button">Delete</button>` : ''}
      </div>
      </div>
    `;
...
document.addEventListener('click',function(e){
 ...
 if(adoptBtn){
   if(!currentUser){ window.location.href='signin.html'; return; }
   const pet=pets.find(p=>String(p.id)===String(adoptBtn.dataset.id));
   if(!pet) return;
   const details=`Name: ${pet.name}
Type: ${pet.type}
Age: ${pet.age}
Location: ${pet.city} ${pet.area}`;
   if(confirm(details+'

Press OK to confirm adoption and open map.')){
      const url=`https://www.google.com/maps?q=${pet.lat||0},${pet.lng||0}`;
      window.open(url,'_blank');
   }
 }
 const delBtn=e.target.closest('.delete-pet-btn');
 if(delBtn){
   pets=pets.filter(p=>String(p.id)!==String(delBtn.dataset.id));
   saveCustomPets();
   renderPets();
 }
});
...
if(useCurrentLocationBtn){
 ...
 useCurrentLocationBtn.addEventListener('click',()=>{
   navigator.geolocation.getCurrentPosition(async pos=>{
     ...
     setPetFormLocation(lat,lng);
     if(petAreaInput) petAreaInput.value=`${lat.toFixed(4)}, ${lng.toFixed(4)}`;
     if(petCityInput) petCityInput.value='Current Location';
   });
 });
}
