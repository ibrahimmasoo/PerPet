...
function renderMessageDashboard(messages) {
  ...
  const unreadMessages = messages.filter(item => item.status !== 'read');
  const unreadMessagesEl = document.getElementById('unreadMessages');
  if (unreadMessagesEl) unreadMessagesEl.textContent = String(unreadMessages.length);
  ...
  messages.forEach(item => {
    ...
    article.className = 'message-card' + (item.status === 'read' ? ' is-read' : '');
    const statusLabel = item.status === 'read' ? 'Read' : 'Unread';
    const toggleLabel = item.status === 'read' ? 'Mark unread' : 'Mark read';
    article.innerHTML = `
      <div class="message-card-head">
        <div>
          <span class="message-badge ${item.status === 'read' ? 'read' : 'unread'}">${statusLabel}</span>
        ...
        <div class="message-side-meta">
          ...
          <div class="message-actions-row">
            <a class="mini-action" href="mailto:${escapeAttribute(item.email || '')}?subject=${encodeURIComponent('Reply from PerPet')}">Reply</a>
            <button class="mini-action" type="button" data-action="toggle-read" data-id="${escapeAttribute(item.id)}">${toggleLabel}</button>
            <button class="mini-action danger" type="button" data-action="delete" data-id="${escapeAttribute(item.id)}">Delete</button>
          </div>
        </div>
      </div>
...
function maybeShowBrowserNotification(message) {
  ...
  const createdTime = message && message.createdAt instanceof Date ? message.createdAt.getTime() : Date.now();
  if (createdTime <= latestMessageTimestamp) return;
  latestMessageTimestamp = createdTime;
  new Notification('New PerPet message', {
    body: `${message.name || 'Visitor'}: ${(message.message || '').slice(0, 80)}`
  });
}
...
async function updateMessageStatus(messageId, nextStatus) {
  ...
  try {
    await db.collection('contactMessages').doc(messageId).update({ status: nextStatus });
    showToast(nextStatus === 'read' ? 'Message marked as read.' : 'Message marked as unread.');
  } catch (error) {
...
function attachDashboardRealtimeListener() {
  if (!dashboardList || !db || !auth || !auth.currentUser) return;
  ...
  messagesUnsubscribe = db.collection('contactMessages').orderBy('createdAt', 'desc').onSnapshot(snapshot => {
    ...
    if (messages.length) {
      maybeShowBrowserNotification(messages[0]);
    }
  }, error => {
    console.error('Dashboard realtime load error:', error);
    if (dashboardLoading) {
      dashboardLoading.classList.remove('hidden-panel');
      dashboardLoading.innerHTML = '<h2>Could not load messages</h2><p>Check Firestore rules and make sure reads are allowed for signed-in users.</p>';
    }
  });
}
...
async function loadMessagesDashboard() {
  ...
  attachDashboardRealtimeListener();
}
...
if (dashboardList) {
  dashboardList.addEventListener('click', event => {
    ...
    const messageId = actionButton.getAttribute('data-id');
    if (!messageId) return;

    if (action === 'toggle-read') {
      const currentlyRead = actionButton.textContent.toLowerCase().includes('unread');
      updateMessageStatus(messageId, currentlyRead ? 'unread' : 'read');
    }
...
if (contactForm) {
  contactForm.addEventListener('submit', async event => {
    ...
    try {
      ...
      contactForm.reset();
      setContactStatus('Your message was sent successfully. It is now saved in the dashboard.', 'success');
