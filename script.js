...
function renderPets() {
  ...
  prepared.forEach(pet => {
    ...
    card.innerHTML = `
      ...
      <div class="pet-body">
        ...
        <button class="btn btn-dark adopt-btn" data-pet-id="${escapeHtml(String(pet.id))}" type="button">Adopt ${escapeHtml(pet.name)}</button>
      </div>
    `;
  ...
  document.querySelectorAll('.adopt-btn').forEach(button => {
    button.addEventListener('click', () => {
      ...
      const selectedPet = pets.find(item => String(item.id) === String(petId));
      if (selectedPet) {
